...
async def login_access_token(
    ...
    if not auth_jwt.verify_password(form_data.password, user.hashed_password):
        raise HTTPException(status_code=400, detail="Incorrect email or password")

    return auth_jwt.generate_access_token_response(str(user.id))
...
async def refresh_token(
    ...
    try:
        payload = jwt.decode(
            ...
            algorithms=[auth_jwt.JWT_ALGORITHM],
        )
